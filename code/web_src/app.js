// 主题固定为浅色(深色模式已移除)
    ...
    function modemAction(action){
      ...
      if(action==='hardreset'){
        if(!confirm('硬重启将通过 GPIO27/EN 给模组断电再上电，确定继续？'))return;
        resultEl.className='result-box result-loading';resultEl.textContent='硬重启中（约15秒）...';
        csrfFetch('/modem?action=hardreset',{method:'POST',cache:'no-store'}).then(jsonOrThrow).then(function(d){
          resultEl.className='result-box '+(d.success?'result-success':'result-error');resultEl.textContent=d.message;
