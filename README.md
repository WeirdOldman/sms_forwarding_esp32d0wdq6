...
- 源码已同步到上游 `master` 的 `a29b97c`（包含 v1.0.9 及后续 mDNS/WiFi 修复）。
- GitHub Actions 工作流 `Build ESP-IDF Firmware` 已成功触发并完成至少一次构建。
- 目标芯片：`esp32`。
- 构建产物已成功打包为 artifact，可用于下载和烧录。

### 本仓库当前尚未完成的验证

- ESP32-D0WDQ6 + ML307R-DC 实机烧录启动验证。
- Wi-Fi 配网 / Web UI 联通验证。
- ML307 AT 通信、驻网、收短信、转发链路验证。
- 长时间老化与弱网稳定性验证。

因此，本仓库当前状态应理解为：**源码移植与 CI 构建已验证通过，实机功能验证待持续补充**。
...
默认引脚定义在 ESP-IDF 模组组件中：TXD=GPIO17，RXD=GPIO16，MODEM_EN=GPIO27，配网按钮=GPIO32。
