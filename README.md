...
<p align="center">
  <a href="https://github.com/WeirdOldman/sms_forwarding_esp32d0wdq6/actions/workflows/build.yml"><img alt="CI" src="https://github.com/WeirdOldman/sms_forwarding_esp32d0wdq6/actions/workflows/build.yml/badge.svg" /></a>
...
ESP32-D0WDQ6 / 经典 ESP32 + ML307R-DC 4G/LTE 模组的轻量短信转发固件。设备通过 UART/AT 接收 PDU 短信，在本地完成中文短信、长短信、去重和规则判断，然后经 WiFi 转发到邮件与最多 5 个推送通道；同时支持来电通知与 SIM 卡热插拔检测，并提供 Web UI 完成配置、诊断、日志、OTA、保号和定时任务。
...
- 设备信息：模组厂家、模组型号、模组固件、主控固件版本、空闲堆、最低堆、最大可分配块、ESP32 片内温度。
...
- WiFi 收发路径、FreeRTOS 与 ringbuf 部分函数放回 flash，释放经典 ESP32 的 IRAM 压力。
...
以下结果来自上游项目在其原始硬件/目标板上的既有验证记录，可作为功能成熟度参考，但**不等同于本仓库 ESP32-D0WDQ6 / ML307R-DC 组合的实机结论**：

- 设备通过串口烧录后运行，Web 访问地址为局域网 IP。
...
### 本仓库（ESP32-D0WDQ6 移植版）当前已验证项

- 已按上游 `master` 的 `9a729ec` 重新适配当前经典 ESP32 硬件配置，保留 GPIO17/GPIO16 UART，并由 GPIO27 控制 ML307 EN。
- ESP-IDF 5.5.4 本地全量构建通过，Web 资源生成一致性检查通过。
- 目标芯片：`esp32`。
- `1.0.9-esp32.12` 已通过 Web OTA 启动，WiFi、Web UI、ML307 AT 通信及 IMEI/ICCID/IMSI/模组固件读取正常；启动日志确认已通过 GPIO27/EN 执行冷启动。
- 自动选网配置为空时不再循环查询或下发 `COPS`；实机启动日志未再出现自动 `COPS` 收敛重试。
- Web 硬重启操作已实机触发 GPIO27/EN 断电上电，ESP32 uptime 未归零；模组约 13 秒恢复 `CEREG=1` 和 PDP，随后 15 秒观察未出现重复 `+MATREADY`。
- 模组恢复后在全功能模式连续观察超过 5 分钟，保持 `CEREG=1`、`CSQ=22`、PDP 地址有效，期间没有新增 `+MATREADY`；同时观察到短信入站与 PDU 合并路径运行。
- 蜂窝射频关闭（`CFUN=4`）后的 180 秒对照观察中，ESP32 uptime 单调增长，日志序号和 `+MATREADY` 数量均未变化。
- 改用独立稳压 5V 外部电源并与 ESP32 共地后，约 64KB 蜂窝 HTTP payload 已返回 HTTP 200，短信接收也已实机成功；旧 USB/VIN 供电阻塞不再成立。
- 发短信诊断中，原 11 位号码和同网服务号均在约 84ms 取得 `>`，完整写入十六进制 PDU 与单个 Ctrl-Z，但随后 120 秒 UART `RX=0`，没有 `+CMGS`、`OK` 或 `+CMS ERROR`。
- 同一 PDU 使用 `AT+CMGW=17` 可写入模组短信存储，`AT+CMGR` 回读 TPDU 完全一致，`AT+CMGD` 删除成功；PDU 编码、TPDU 长度、UART 写入、Ctrl-Z 和模组本地解析已排除。
- `.12` 区分 AT 通道忙与命令超时，记录 CMGS 各阶段证据；发送事务超时不自动重发，并使用 GPIO27/EN 单次硬重启，避免 `CFUN` 软重启造成连续 `+MATREADY`。

### 本仓库当前尚未完成的验证

- 移动发短信（MO-SMS）仍未通过：`CEREG=1` 只证明 EPS 注册，不能证明当前 SIM 已开通可用的 MO-SMS 承载。`AT+CGSMS=?` 在当前 ML307 固件上只报告模式 3，无法通过固件切换承载做 A/B。
- 下一项必须在运营商/SIM 侧验证：把同一张 SIM 放入手机发送普通短信，确认套餐余额、主动发短信权限和短信中心号码。若手机同样失败，应由运营商处理；若手机成功而 ML307 仍 60 秒无任何终态，阻断点在 ML307 协议栈或其与当前网络的兼容性。
- 完整的“收到短信 -> 规则匹配 -> 邮件/推送到达”链路尚未配置实际目标做端到端验收。
- 长时间老化与弱网稳定性验证。

因此，本仓库当前状态应理解为：**最新上游源码适配、本地构建、OTA、GPIO27/EN 控制、Web、AT/身份读取、驻网、PDP、蜂窝 payload 和短信接收已验证；主动发短信已证明固件完成本地 PDU 提交，但 ML307/运营商侧没有返回 MO-SMS 网络终态，不能宣称该功能已通过验收**。
...
- Windows + PowerShell。
- ESP32 开发板（推荐 ESP32 Type-C CH340 / ESP32-D0WDQ6，4MB Flash）。
- ML307 系列 4G/LTE 模组。
- USB 数据线和对应串口驱动。
...
```powershell
git clone https://github.com/WeirdOldman/sms_forwarding_esp32d0wdq6.git
cd sms_forwarding_esp32d0wdq6
```
...
如果自动进入下载模式失败，可以按住开发板 `BOOT`，轻点 `RST/EN`，开始烧录后松开 `BOOT`。不同 ESP32 开发板按钮名称可能不同，以板子丝印为准。
...
ESP32 Type-C CH340 / ESP32-D0WDQ6 与 ML307x-DC UART 连接：

```text
ESP32 GPIO17 -> ML307 RX
ESP32 GPIO16 -> ML307 TX
ESP32 GND    -> ML307 GND
ESP32 GPIO27 -> ML307 EN

独立稳压 5V 正极 -> ML307 VCC (+5V)
独立稳压 5V 负极 -> ML307 GND（与 ESP32 GND 共地）
```

ML307 EN 只连接 ESP32 GPIO27，**不要再同时连接 +5V**。固件以高电平启用模组，硬重启时将
GPIO27 拉低 1.2 秒后再拉高；Web 仍提供显式 AT 软重启，但短信提交超时和 SIM 插入恢复使用
GPIO27/EN 硬重启，避免 ML307 在 `AT+CFUN=1,1` 后连续上报 `+MATREADY`。默认引脚定义为
TXD=GPIO17、RXD=GPIO16、EN=GPIO27、配网按钮=GPIO32。

ESP32 可继续由 USB 单独供电，但不要再从 ESP32 VIN/5V 给 ML307 供电，也不要把外部 5V 反灌到
ESP32 VIN。ML307 电源建议使用持续输出能力不低于 2A 的稳压 5V 电源；供电线应短且足够粗。
