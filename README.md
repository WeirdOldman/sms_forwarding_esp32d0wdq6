...
ESP32-D0WDQ6 / 经典 ESP32 + ML307 系列 4G/LTE 模组的轻量短信转发固件。设备通过 UART/AT 接收 PDU 短信，在本地完成中文短信、长短信、去重和规则判断，然后经 WiFi 转发到邮件与最多 5 个推送通道；同时支持来电通知与 SIM 卡热插拔检测，并提供 Web UI 完成配置、诊断、日志、OTA、保号和定时任务。
...
- WiFi 配网：支持首次 SoftAP 配网，正常连接后可通过 IP 或 `sms.local` 访问。
- 账号密码：Web 管理账号可单独保存。
- 时间设置：NTP 服务器和时区可配置。
...
- 支持 SoftAP 配网、`sms.local` mDNS、NTP 时间同步、AT 终端和运行状态查看。
...
以下结果来自上游项目在其原始硬件/目标板上的既有验证记录，可作为功能成熟度参考，但**不等同于本仓库 ESP32-D0WDQ6 / ML307DC 组合的实机结论**：
...
首次启动时设备会优先连接已保存 WiFi；如果没有配置或连接失败，会开启开放热点 `SMS-Forwarder-XXXX`。连接热点后访问 `http://192.168.1.1` 配网；设备连上路由器后，可通过串口日志里的 IP 地址或 `http://sms.local` 打开 Web UI。
